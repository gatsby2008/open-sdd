...
| **`/doc-publish`** | Publish the current `service-info.md` to the central registry (`$CLAUDE_DOC_HOME/service-catalog/`, default `~/.claude/service-catalog/`). Use `list` to inspect what is registered. |
...
| **`/adr-publish`** | Publish all ADRs from the current project to the central registry (`$CLAUDE_DOC_HOME/adr-registry/<service>/`). Clear-and-copy: replaces the entire service subdir. |
...
| Variable | Effect |
|----------|--------|
| `$CLAUDE_DOC_HOME` | Overrides the registry root for team-shared registries (e.g., a cloned GitLab repo). Affects all four doc skills: `/doc-publish`, `/doc-query`, `/adr-publish`, `/adr-query`. |
