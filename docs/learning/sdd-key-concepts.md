...
| Concept | Summary |
|---------|---------|
| **AGENTS.md as dual-source** | opencode loads `AGENTS.md` as system instructions (full prose, section semantics). `/f-start` compiles the project's `AGENTS.md` + `.opensdd/service-rules.md` into `rules.json` via `engine/gates.py` (lossy: only top-level bullets, capped at 20, no sub-bullets). Two different consumers, one source file. |
